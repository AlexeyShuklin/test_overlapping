...
export default class LabelOverlapping extends Component {
	...
	componentDidUpdate(prevProps, prevState) {
		...
		for (let i = 0; i < elements.length; i++) {
			...
			for (let j = 0; j < elements.length; j++) {
				if(elements[j] != currentLabel && elements[j].style.display!='none') {
					let overlapLabel = elements[j].getBoundingClientRect();
					if(((Math.abs(currentLabelRect.left - overlapLabel.left) * 2 < (currentLabelRect.width + overlapLabel.width + 12)) &&
					 (Math.abs(currentLabelRect.top - overlapLabel.top) * 2 < (currentLabelRect.height + overlapLabel.height+6)))) {
						// overlap, hide label
						elements[j].style.display = 'none'
					}
				}
	...
	render() {
		...
		return (
			<div>
			<div>Original</div>
			<svg ref='orig' className="Dots" width={ width } height={ height }>
			{
				points.map((d, i) => {
					return [
						<circle className="Circle"
							key={ i }
							cx={ x(d[0]) } cy={ y(d[1]) }
							r="10" fill={colors(i)}></circle>,
						<text className="place-label"
							x={ x(d[0])+12}
							y={ y(d[1])} style={{display: true}}
						>{ 'label' + i}</text>]
				})
			}
			</svg>
